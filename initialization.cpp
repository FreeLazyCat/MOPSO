...
void initializeParticle(Particle &particle){
    ...
    int len = getMax(lenP, lenO);

    particle.Velocity = new double[len];
    for (int i=0; i < len; i++)
        particle.Velocity[i] = 0;
    particle.sizeOfVelocity = len;
    ...
    particle.Best.Cost = new double[numObjective];
    cpyDoubleArray(particle.Best.Cost, particle.Cost, numObjective);
}